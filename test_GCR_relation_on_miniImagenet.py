...
learning_rate = 1e-3
# Options
checkpoint = '/home/liweijie/projects/few-shot/checkpoint/20200332_miniImage_GCR_r_best.pth.tar'
log_interval = 20
device_list = '0'
num_workers = 8
...
for epoch in range(start_epoch, start_epoch+1):
    acc = test_100way(model,criterion,val_loader,device,epoch,log_interval,writer,args)
    print('Batch accu_a on miniImagenet: {:.3f}'.format(acc))
    acc = test_100way(model,criterion,val_loader2,device,epoch,log_interval,writer,args)
    print('Batch accu_b on miniImagenet: {:.3f}'.format(acc))
    acc = test_100way(model,criterion,val_loader3,device,epoch,log_interval,writer,args)
